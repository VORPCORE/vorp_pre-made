...
Animations.playAnimation = function(ped, anim)
    ...
    if animation.prop then
        ...
        AttachEntityToEntity(mainprop, ped, boneIndex, animation.prop.coords.x, animation.prop.coords.y,
            animation.prop.coords.z, animation.prop.coords.xr, animation.prop.coords.yr, animation.prop.coords.zr, true,
            true, false, true, 1, true, false, false)

        if animation.prop.subprop then
            local pcoords = GetEntityCoords(subprop)
            subprop = CreateObject(animation.prop.subprop.model, pcoords.x, pcoords.y, pcoords.z, true, true, false,
                false, true)
            AttachEntityToEntity(subprop, ped, boneIndex, animation.prop.subprop.coords.x,
                animation.prop.subprop.coords.y, animation.prop.subprop.coords.z, animation.prop.subprop.coords.xr,
                animation.prop.subprop.coords.yr, animation.prop.subprop.coords.zr, true, true, false, true, 1, true,
                false, false)
        end
    end
    ...
    if animation.type == 'scenario' then
        TaskStartScenarioInPlaceHash(ped, GetHashKey(animation.hash), 12000, true, 0, 0, false)
    elseif animation.type == 'standard' then
        ...
        TaskPlayAnim(ped, animation.dict, animation.name, 1.0, 1.0, -1, animation.flag, 1.0, false, 0, false, '', false)
    end
end
