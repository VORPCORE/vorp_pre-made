...
InventoryService.DropMoney = function(amount)
    ...
    if not SvUtils.InProcessing(_source) then
        ...
        if Config.NewPlayers then
            if contains(newchar, charid) then
                -- new line
...
InventoryService.canStoreWeapon = function(identifier, charIdentifier, invId, name, amount)
    ...
    if invData.limit > 0 then
        -- if inventory limit is bigger than 0
        ...
        if sourceInventoryItemCount > invData.limit then
            -- if its less or equals  then return true
            return false
        end
    end

    if invData.limitedWeapons[string.lower(name)] then
        -- if it does exists in the table the weapns name
        local weapons = SvUtils.FindAllWeaponsByName(invId, name)  -- find name
        local weaponCount = #weapons + amount                      -- do the math
        if weaponCount > invData.limitedWeapons[string.lower(name)] then
            -- if ammount is greater then cancel
            return false
        end
    elseif invData.whitelistWeapons then
        -- if name isnt on the list and choose whitelist items reject, otherwise say you can store any weapon if whitelist is false
        return false
    end
...
InventoryService.DoesHavePermission = function(invId, job, grade, Table)
    if not CustomInventoryInfos[invId].UsePermissions then
        -- allow everyone if false or nil by fefault is false
        return true
    end

    if not next(Table) then
        -- if empty allow anyone by default is empty
        return true
    end
...
InventoryService.CheckIsBlackListed = function(invId, ItemName)
    ...
    if not CustomInventoryInfos[invId].UseBlackList then
        -- if not want to black list
        return true
    end
