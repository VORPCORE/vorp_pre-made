...
InventoryAPI.getUserWeapon = function(player, cb, weaponId)
    ...
    if foundWeapon then
        ...
        wrappedWeapon.id = foundWeapon:getId()
        wrappedWeapon.propietary = foundWeapon:getPropietary()
        wrappedWeapon.used = foundWeapon:getUsed()
        wrappedWeapon.ammo = foundWeapon:getAllAmmo()
        wrappedWeapon.desc = foundWeapon:getDesc()
    end
...
InventoryAPI.setItemMetadata = function(player, itemId, metadata, amount, cb)
    ...
    if amountRemove >= count then
        -- if greater or equals we set meta data
        ...
        -- we set meta data to only the amount we want
