...
AddEventHandler('vorp_bank:getinfo', function(name)
    ...
    exports.oxmysql:execute("SELECT * FROM bank_users WHERE charidentifier = @charidentifier AND name = @name"
...
AddEventHandler('vorp_bank:UpgradeSafeBox', function(costlot, maxslots, slotsBought, name, currentspace)
    ...
    if FinalSlots > maxslots then
        TriggerClientEvent("vorp:TipRight", _source,
            Config.language.maxslots .. " | " .. slotsBought .. " / " .. maxslots, 10000)
        return
    end
    ...
    exports.oxmysql:execute(
        "UPDATE bank_users SET invspace=@invspace WHERE charidentifier=@charidentifier AND name = @name"
        , Parameters)

    TriggerClientEvent("vorp:TipRight", _source,
        Config.language.success .. (costlot * slotsBought) .. " | " .. FinalSlots .. " / " .. maxslots, 10000)
end)
    ...
    if money >= amount then
        exports.oxmysql:execute("SELECT * FROM bank_users WHERE charidentifier = @charidentifier AND name = @name"
            ...
            if result[1] then
                ...
                Wait(100)
                TriggerClientEvent("vorp_bank:ReloadBankMenu", _source, bankinfos, name)
...
AddEventHandler('vorp_bank:depositgold', function(amount, name, bankinfo)
    ...
    if money >= amount then
        ...
        Wait(100)
        TriggerClientEvent("vorp_bank:ReloadBankMenu", _source, bankinfos)
...
AddEventHandler('vorp_bank:withgold', function(amount, name, bankinfo)
    ...
    local Character = VorpCore.getUser(_source).getUsedCharacter
    local charidentifier = Character.charIdentifier
    exports.oxmysql:execute("SELECT gold FROM bank_users WHERE charidentifier = @charidentifier AND name = @name"
        ,
        { ["@charidentifier"] = charidentifier,["@name"] = name }, function(result)
        local gold = result[1].gold
        if gold >= amount then
            ...
            TriggerClientEvent("vorp_bank:ReloadBankMenu", _source, bankinfo, name)
        else
            TriggerClientEvent("vorp:TipRight", _source, Config.language.invalid, 10000)
        end
    end)
...
AddEventHandler("vorp_bank:find", function(name)
    local _source = source
    exports.oxmysql:execute('SELECT * FROM bank_users', {}, function(result)
        local banklocations = {}
        if result[1] then
            for i = 1, #result, 1 do
                                    ...
                                    if countDB > space then
                                        ...
                                        if itemType == "item_standard" then
                                            if dataMeta then
                                                ...
                                                TriggerClientEvent("vorp:TipRight", _source,
                                                    Config.language.depoitem3 ..
                                                    itemCount .. Config.language.of .. item.label, 5000)
                                            else
                                        ...
                                        if itemType == "item_weapon" then
                                            ...
                                            TriggerClientEvent("vorp:TipRight", _source,
                                                Config.language.depoitem3 .. item.label, 5000)
                                        end
                        ...
                        if result[1].items then
                            ...
                            if countDB > space then
                                ...
                                if itemType == "item_standard" then
                                    ...
                                    TriggerClientEvent("vorp:TipRight", _source,
                                        Config.language.depoitem3 .. itemCount .. Config.language.of .. item.label, 5000)
                                end
                                if itemType == "item_weapon" then
                                    local weapId = item.id
                                    VorpInv.subWeapon(_source, weapId)
                                    TriggerClientEvent("vorp:TipRight", _source, Config.language.depoitem3 .. item.label
                                        , 5000)
                                end
                                TriggerClientEvent("vorp_inventory:ReloadBankInventory", _source, json.encode(items))
                                exports.oxmysql:execute(
